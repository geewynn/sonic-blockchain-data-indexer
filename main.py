...
async def process_blocks_range(
    start_block_blocks,
    start_block_receipts,
    start_block_logs,
    start_block_traces,
    end_block,
    batch_size,
    ...
    for batch_idx in tqdm.tqdm(range(total_batches), desc="Processing block batches"):
        ...
        block_tasks = [
            process_block(block_num, url, semaphore)
            for block_num in range(batch_start, batch_end + 1)
            if block_num >= start_block_blocks
        ]
        receipt_tasks = [
            process_receipt(block_num, url, semaphore)
            for block_num in range(batch_start, batch_end + 1)
            if block_num >= start_block_receipts
        ]
        log_task = process_logs(
            batch_start if batch_start >= start_block_logs else start_block_logs,
            batch_end,
            url,
            semaphore,
            micro_batch_size=MICRO_BATCH_SIZE,
        )
        trace_task = process_traces(
            batch_start if batch_start >= start_block_traces else start_block_traces,
            batch_end,
            url,
            semaphore,
            micro_batch_size=MICRO_BATCH_SIZE,
        ...
        flat_receipts = [
            receipt for sublist in batch_receipts if sublist for receipt in sublist
        ]
        ...
        if batch_blocks:
            ...
            block_dir = get_partition_path(
                output_dir,
                BLOCKS_FOLDER,
                safe_block_number(first_block.get("number")),
                safe_block_number(last_block.get("number")),
            )
            blocks_path = write_batch_to_parquet(
                batch_blocks, BLOCKS_FOLDER, batch_counter, block_dir
            )
            if blocks_path:
        ...
        if flat_receipts:
            ...
            receipt_dir = get_partition_path(
                output_dir,
                RECEIPT_FOLDER,
                safe_block_number(first_receipt.get("blockNumber")),
                safe_block_number(last_receipt.get("blockNumber")),
            )
            receipts_path = write_batch_to_parquet(
                flat_receipts, RECEIPT_FOLDER, batch_counter, receipt_dir
            )
            if receipts_path:
                upload_to_storage(
                    receipts_path, os.path.relpath(receipts_path, output_dir)
                )
            save_checkpoint(
                "receipts", safe_block_number(last_receipt.get("blockNumber"))
            )

        if batch_logs:
            first_log = batch_logs[0]
            last_log = batch_logs[-1]
            logs_dir = get_partition_path(
                output_dir,
                LOGS_FOLDER,
                safe_block_number(first_log.get("blockNumber")),
                safe_block_number(last_log.get("blockNumber")),
            )
            logs_path = write_batch_to_parquet(
                batch_logs, LOGS_FOLDER, batch_counter, logs_dir
            )
            if logs_path:
        ...
        if batch_traces:
            ...
            traces_dir = get_partition_path(
                output_dir,
                TRACES_FOLDER,
                safe_block_number(first_trace.get("blockNumber")),
                safe_block_number(last_trace.get("blockNumber")),
            )
            traces_path = write_batch_to_parquet(
                batch_traces, TRACES_FOLDER, batch_counter, traces_dir
            )
            if traces_path:
...
async def main():
    ...
    overall_start = min(
        start_block_blocks, start_block_receipts, start_block_logs, start_block_traces
    )
    end_block = overall_start + END_BLOCK
    print(f"Processing blocks from {overall_start} to {end_block}")

    await process_blocks_range(
        start_block_blocks,
        start_block_receipts,
        start_block_logs,
        start_block_traces,
